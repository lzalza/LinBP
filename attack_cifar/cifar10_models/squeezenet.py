...
class SqueezeNet(nn.Module):
    def __init__(self, input_channel=3, n_classes=10):
        super().__init__()
        self.name = 'resnet'
...
def squeezenet(pretrained=False, progress=True, device='cpu', **kwargs):
    model = SqueezeNet(**kwargs)
    if pretrained:
        script_dir = os.path.dirname(__file__)
        state_dict = torch.load(script_dir+'/state_dicts/squeezenet.pt', map_location=device)
