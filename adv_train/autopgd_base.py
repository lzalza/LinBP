...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    logits = self.model(x_adv, self.LinBP)
        ...
        k = self.n_iter_2 + 0
        n_fts = 1
        for i in range(len(self.orig_dim)):
            n_fts = n_fts * self.orig_dim[i]
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_1,
                        x - self.eps), x + self.eps), -10.0, 10.0)
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        if not self.is_tf_model:
            y_pred = self.model(x,self.LinBP).max(1)[1]
        else:
        ...
        for target_class in range(2, self.n_target_classes + 2):
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.is_tf_model:
                        output = self.model(x_to_fool,self.LinBP)
                    else:
