...
use_cuda=True
device = torch.device("cuda:"+str(GPU_NUMBER) if (use_cuda and torch.cuda.is_available()) else "cpu")
# Read eval image
...
def select_model(model, pretrained = False, use_cuda = True):
    ...
    if model == "vgg16_bn":
        ...
        return WRN(pretrained = pretrained, device = device)
    elif model == "wrn28-10":
        return WRN(pretrained = pretrained, device = device)
    elif model == 'squeezenet':
        return squeezenet(pretrained = pretrained, device = device)
...
def generate_saving_image(model, data, labels, eps, alpha, attack_type, K, LinBP):
    ...
    saving, accuracy, normal_accuracy = attacker.attack(eps, alpha, attack_type, saving_mode = True, K = K, LinBP = LinBP)
...
def adversarial_read_gen_save(image_path, model, eps, alpha, attack_type, adv_output, K, LinBP=False):
    ...
    saving, accuracy, normal_accuracy = generate_saving_image(model, data, labels, eps, alpha, attack_type, K, LinBP)
...
class Attacker:
    ...
    def pgd_attack(self, image, ori_image, eps, alpha, data_grad) :
        #print(data_grad.sign().size())
        #print(data_grad.norm().size())
        adv_image = image + alpha * data_grad.sign()
        #adv_image = image + alpha * data_grad / data_grad.norm() * 32 * 32 * 3
        eta = torch.clamp(adv_image - ori_image.data, min=-2 * eps, max=2 * eps)
        image = ori_image + eta

        return image

    def attack(self, epsilon, alpha, attack_type = "FGSM", saving_mode = False, K=10, LinBP=False):
        adv_all = []
        ...
        for now, (data, target) in enumerate(self.loader):
            if(now % 1 == 0):
                print(str(now) + "|" +str(len(self.loader)))
            data, target = data.to(device), target.to(device)
            data_raw = data
            # initial prediction
            if(self.model.name == 'WRN'):
                std_ = torch.tensor(self.std).to(device)
                mean_ =  torch.tensor(self.mean).to(device)
                std_ = std_.unsqueeze(0)
                std_ = std_.unsqueeze(2)
                std_ = std_.unsqueeze(3)
                std_ = std_.expand(data.shape)
                mean_ = mean_.unsqueeze(0)
                mean_ = mean_.unsqueeze(2)
                mean_ = mean_.unsqueeze(3)
                mean_ = mean_.expand(data.shape)
                data = data * std_ + mean_
                output = self.model(data,LinBP=LinBP)
            else:
                output = self.model(data,LinBP=LinBP)
            ...
            if attack_type == "FGSM":
                ...
                data = data + data.new(data.size()).uniform_(-epsilon,epsilon)
                for i in range(K):
                    data.requires_grad = True
                    output = self.model(data,LinBP=LinBP)
                    #print(output.sum())
                ...
                seed_ = time.time()
                self.apgd = APGDAttack(self.model, n_restarts=1, n_iter=100, verbose=False,
                eps=epsilon, norm='Linf', eot_iter=1, rho=.75, seed=seed_,
                device=device,LinBP = LinBP)
                self.apgd.loss = 'ce'
                perturbed_data = self.apgd.perturb(data,target)
            ############ ATTACK GENERATION ##############

            # prediction of adversarial image
            output = self.model(perturbed_data,LinBP=LinBP)
